...
class CRF(nn.Module):
    ...
    def _viterbi_decode(self, feats, mask):
        ...
        for idx, cur_values in seq_iter:
            ...
            prb = self.softmax(cur_values)
            cur_values = cur_values + partition.contiguous().view(batch_size, tag_size, 1).expand(batch_size, tag_size, tag_size)
            ## forscores, cur_bp = torch.max(cur_values[:,:-2,:], 1) # do not consider START_TAG/STOP_TAG
            # print "cur value:", cur_values.size()
            partition, cur_bp = torch.max(cur_values, 1)
            prb = torch.gather(prb, 1, cur_bp.unsqueeze(1))
            probLists.append(prb.squeeze(1))
        ...
        pad_zero1 = autograd.Variable(torch.zeros(batch_size, tag_size)).float()
        if self.gpu:
            pad_zero = pad_zero.cuda()
            pad_zero1 = pad_zero1.cuda()
        back_points.append(pad_zero)
        probLists.append(pad_zero1)
        back_points  =  torch.cat(back_points).view(seq_len, batch_size, tag_size)
        probLists = torch.cat(probLists).view(seq_len, batch_size, tag_size)
        ...
        decode_prob = autograd.Variable(torch.FloatTensor(seq_len, batch_size))

        if self.gpu:
            decode_idx = decode_idx.cuda()
            decode_prob = decode_prob.cuda()
        decode_idx[-1] = pointer.data
        decode_prob[-1] = torch.gather(probLists[-1], 1, pointer.contiguous().view(batch_size,1)).view(batch_size).data
        for idx in range(len(back_points)-2, -1, -1):
            pointer = torch.gather(back_points[idx], 1, pointer.contiguous().view(batch_size,1)).view(batch_size)
            dec_prob = torch.gather(probLists[idx], 1, pointer.contiguous().view(batch_size,1)).view(batch_size)
            decode_idx[idx] = pointer.data
            decode_prob[idx] = dec_prob.data
        path_score = None
        decode_idx = decode_idx.transpose(1,0)
        decode_prob = decode_prob.transpose(1,0)
        return decode_prob, decode_idx
