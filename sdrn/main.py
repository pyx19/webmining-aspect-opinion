...
def bert_load_state_dict(model, state_dict):
    ...
    def load(module, prefix=''):
        local_metadata = {} if metadata is None else metadata.get(prefix[:-1], {})
        # Updated call to _load_from_state_dict
        module._load_from_state_dict(
    ...
    # Log missing and unexpected keys if necessary
    if missing_keys:
        print(f"Missing keys: {missing_keys}")
    if unexpected_keys:
        print(f"Unexpected keys: {unexpected_keys}")
    if error_msgs:
        raise RuntimeError(f"Errors occurred when loading state_dict: {error_msgs}")
    ...
    if args.mode == "test":
        ...
        for idx in range(args.iteration):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                sample_loss += rloss.data.item() + tloss.data.item()

                # total_loss += rloss.data[0] + tloss.data[0]
                total_loss += rloss.data.item() + tloss.data.item()
